package org.netbeans.modules.jeeserver.ant.base.embedded.project.nodes;
...
public class Html5RefActions {
    ...
    public static boolean assignHtml5ExternalServer(Project serverProject, FileObject webappFo) {
        ...
        //
        Properties html5Props = BaseUtils.loadProperties(webappFo.getFileObject("nbproject/project.properties"));
...
//                Html5RefActions.assignHtml5ExternalServer(serverProject, html5ProjFo);
    ...
    public static class HtmRefShowBrowserAction extends AbstractAction implements ContextAwareAction {
        ...
        private static final class ContextAction extends AbstractAction implements ProgressListener {
            ...
            public @Override
            void actionPerformed(ActionEvent e) {
                Properties props = BaseUtils.loadProperties(htmrefFo);
                if (props == null) {
                    return;
                }
                webAppLocation = props.getProperty(EmbConstants.WEB_APP_LOCATION_PROP);
            ...
            void show() {
                try {
                    HtmRefDeployAction.perform(dm, contextPath, webAppLocation);
                    BaseUtils.out("ShowBrowser: urlStr=" + urlStr);
                    HtmlBrowser.URLDisplayer.getDefault().showURL(new URL(urlStr));
                } catch (MalformedURLException ex) {
                    BaseUtils.out("ShowBrowser: urlStr=" + urlStr + "; EXCEPTION "  + ex.getMessage());                    
